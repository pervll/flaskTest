    ...
    for (let i in chessPiece){
        for (let j in i){
            for (k=0;k<j.length;k++){
                let (i+j+k) = new component(60,60,"/static/resources/"+(i+j)+".svg",(k[0]-1)*80,(k[1]-1)*80,image)
            }
        }
    }
}
function create_scene(){
    let board = new component(640,640,"/static/resources/chessboard.jpg",0,0,image)
    create_piece()
}
function init(){
    gameArea.init()
